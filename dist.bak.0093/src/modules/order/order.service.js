...
let OrderService = OrderService_1 = class OrderService {
    ...
    constructor(prisma, cartService, promotionService, trackingService, pointService, ghnService, paymentService, charityService, systemSetting, notificationService, redis) {
        this.prisma = prisma;
    ...
    async previewOrder(userId, dto) {
        ...
        if (dto.useCoins) {
            ...
            const requested = Number.isFinite(dto.appliedCoins)
    ...
    async cancelOrder(userId, orderId) {
        ...
        if (order.shippingOrderCode) {
            try {
                await this.ghnService.cancelShippingOrder(order.shippingOrderCode);
            }
            catch (e) {
                this.logger.warn(`[GHN void cancel] order=${orderId} err=${e?.message}`);
            }
        }
        return updatedOrder;
    }
    ...
    async updateOrderStatus(orderId, sellerId, status) {
        ...
        const _rank = { PENDING: 0, CONFIRMED: 1, SHIPPING: 2, DELIVERED: 3 };
        if (status !== 'CANCELLED' && _rank[status] < _rank[order.status]) {
            throw new common_1.BadRequestException('Không thể lùi trạng thái đơn hàng.');
        }
