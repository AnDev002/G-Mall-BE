...
export class OrderService {
  ...
  async createOrder(userId: string, dto: CreateOrderDto) {
    // Gọi preview để lấy số liệu chính xác (bao gồm phí ship GHN vừa tính)
    ...
    if (order && dto.paymentMethod === 'cod') {
        try {
            const receiver = dto.receiverInfo || {};
            // Chuẩn bị dữ liệu gửi sang GHN
            const ghnOrderData = {
                to_name: order.recipientName,
                to_phone: order.recipientPhone,
                to_address: order.recipientAddress,
                to_ward_code: receiver['wardCode'],     // [QUAN TRỌNG] Cần có trường này
                to_district_id: Number(receiver['districtId']), // [QUAN TRỌNG] Cần có trường này
                cod_amount: order.totalAmount, // Thu hộ bằng tổng tiền đơn hàng
