...
export class ProductReadService implements OnModuleInit {
  ...
  async findOnePublic(idOrSlug: string) {
    // 1. Kiểm tra Cache (Ưu tiên tốc độ)
    // Gọi cache service để tìm xem có data lưu dưới key ID hoặc Slug không
    const cachedProduct = await this.productCache.getProductDetail(idOrSlug);
    
    // Nếu có cache và trạng thái Active -> Trả về luôn (Siêu nhanh)
    if (cachedProduct && cachedProduct.status === 'ACTIVE') {
         return cachedProduct; 
    }

    // 2. Nếu không có Cache -> Tìm trong DB
    // [FIX] Sử dụng mode 'insensitive' cho slug để 'Ao-Thun' và 'ao-thun' đều tìm thấy nhau
    const product = await this.prisma.product.findFirst({
      where: {
        OR: [
            { id: idOrSlug }, 
            { slug: idOrSlug } 
            // Nếu DB là Postgres, có thể dùng: { slug: { equals: idOrSlug, mode: 'insensitive' } }
        ],
      },
      include: {
