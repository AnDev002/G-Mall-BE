...
export class ProductReadService implements OnModuleInit {
  ...
  async findAllPublic(query: FindAllPublicDto) {
    ...
    // 1. Ưu tiên Search bằng Redis (Nếu có keyword)
    if (query.search && query.search.trim().length > 0) {
        try {
            // Xử lý keyword:
            // - "Kem đánh răng" -> Tìm (Kem | %Kem%) AND (đánh | %đánh%) ... cho phép sai chính tả
            // - Escape các ký tự đặc biệt của RediSearch
            const cleanKeyword = query.search.replace(/[@!{}()|<>*]/g, '');
            const terms = cleanKeyword.trim().split(/\s+/);
            
            // Query String cho RediSearch: 
            // Vd: "@status:{ACTIVE} @name:(Kem*|%Kem%*) @name:(đánh*|%đánh%*)"
            // Fuzzy search: %term% cho phép sai 1 ký tự
            const termQuery = terms.map(t => `( ${t}* | %${t}% )`).join(' ');
            
            const ftQuery = `@status:{ACTIVE} @name:${termQuery}`;

            // Execute lệnh FT.SEARCH
            // RETURN: id để query DB hoặc hydrate data
            ...
            // searchRes format: [total_count, key1, val1, key2, val2...]
            const total = searchRes[0];
            const productIds: string[] = [];
            
            // Parse kết quả từ mảng phẳng
            for (let i = 1; i < searchRes.length; i += 2) {
                const key = searchRes[i]; // "product:uuid..."
                const id = key.replace('product:', '');
                productIds.push(id);
            }

            if (productIds.length > 0) {
                // Hydrate full data từ Cache Service hoặc DB
                const products = await this.productCache.getProductsByIds(productIds);
                return {
                    data: products,
            ...
            // Nếu Redis trả về 0 kết quả, Fallback xuống DB bên dưới (để chắc chắn)
        } catch (e) {
            this.logger.error('RediSearch failed, falling back to DB', e);
        }
    }

    // 2. Fallback: Logic Prisma cũ (nhưng tối ưu lại query LIKE)
    ...
    // [FIX LOGIC DATABASE] Chia nhỏ keyword để tăng độ chính xác
    if (query.search) {
        const keywords = query.search.trim().split(/\s+/).filter(k => k.length > 0);
        if (keywords.length > 0) {
            // Thay vì name contains "full string", ta dùng AND: name chứa "Kem" VÀ chứa "đánh"
            where.AND = keywords.map(word => ({
                name: { contains: word } // MySQL default case-insensitive
            }));
        }
    }

    // ... Giữ nguyên các logic filter price, rating, category của bạn ...
    ...
    const [productIdsData, total] = await Promise.all([
      this.prisma.product.findMany({
        ...
        take: limit,
        skip: skip,
        orderBy: { createdAt: 'desc' }, // Hoặc sort theo query
        select: { id: true },
      }),
