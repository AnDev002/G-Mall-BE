...
export class AdminProductController {
  ...
  @Query('categoryId') categoryId: string
   ...
   // 2. Filter theo Search (ĐÃ SỬA)
   if (search) {
       whereCondition.OR = [
           { name: { contains: search } }, 
           { slug: { contains: search } }, // Thêm tìm theo slug (như gợi ý của Prisma)
           // Tìm sản phẩm có ít nhất 1 biến thể chứa SKU khớp từ khóa
           { variants: { some: { sku: { contains: search } } } } 
       ];
   }

   // 3. Filter theo Category (ĐÃ SỬA TRƯỚC ĐÓ)
   if (categoryId) {
