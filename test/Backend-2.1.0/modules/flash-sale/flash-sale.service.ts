...
import { FlashSaleSession, Prisma, FlashSaleProductStatus } from '@prisma/client';
...
export class FlashSaleService {
  ...
  async findAvailableSessionsForSeller() {
    const now = new Date();
    // Logic: Lấy các session sắp diễn ra hoặc đang diễn ra
    const sessions = await this.prisma.flashSaleSession.findMany({
      where: {
        status: 'ENABLED',
        endTime: { gt: now }, // Chưa kết thúc
      },
  ...
  async registerProducts(sellerId: string, dto: RegisterFlashSaleDto) {
    ...
    for (const item of items) {
      ...
      // Ép kiểu variant.price (Decimal) về Number trước khi so sánh
      if (item.promoPrice >= Number(variant.price)) {
         continue; 
      }
      ...
      const record = await this.prisma.flashSaleProduct.upsert({
        ...
        update: {
          // FIX: Sửa 'price' -> 'salePrice'
          salePrice: item.promoPrice,
          stock: item.promoStock,
          // FIX: Sửa 'ACCEPTED' -> 'APPROVED'
        ...
        create: {
          ...
          variantId: item.variantId,
          // Thêm originalPrice (lấy từ variant) cho đầy đủ dữ liệu
